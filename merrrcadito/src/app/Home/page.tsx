            ...
            <div className={styles.scrollWrapper}>
                ...
                <div ref={scrollContainerRef} className={styles.scrollContainer}>
                    {tokenPackages.map((pkg: any) => (
                        <TokenPackageCard key={pkg.id} pkg={pkg} tokenStyles={tokenStyles} />
                    ))}
...
const TokenPackageCard = ({ pkg, tokenStyles }: { pkg: any, tokenStyles: any }) => {
    const [imgError, setImgError] = React.useState(false);
    const imageUrl = `${process.env.NEXT_PUBLIC_API_URL}/token_package/${pkg.id}/image`;

    return (
        <div className={tokenStyles.card} style={{ minWidth: '280px' }}>
            <div className={tokenStyles.imagePlaceholder}>
                {!imgError ? (
                    ...
                    <div style={{
                        ...
                        <i className="bi bi-coin" style={{ fontSize: '3rem', color: '#FFD700' }}></i>
                    </div>
                )}
            </div>
            <div className={tokenStyles.info}>
                <h3 className={tokenStyles.title}>{pkg.nombre}</h3>
                <p className={tokenStyles.tokens}>{pkg.tokens} Tokens</p>
                <p className={tokenStyles.price}>{pkg.precio} Bs.</p>
            </div>
        </div>
    );
};